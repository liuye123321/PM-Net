        ...
        for key in self.keys:
            ...
            if isinstance(img, list):
                for i in range(len(img)):
                    results[key][i] = to_tensor(img[i].transpose(2, 0, 1))
            else:
                if len(img.shape) < 3:
...
class DefaultFormatBundle(object):
    ...
    def __call__(self, results):
        ...
        if 'gt' in results:
            ...
            results['gt'] = DC(to_tensor(gt), stack=True)
        if 'depth' in results:
            depth = results['depth']
        ...
        if 'ref' in results:
            for i in range(len(results['ref'])):
                ...
                ref = np.ascontiguousarray(ref.transpose(2, 0, 1))
                results['ref'][i] = DC(to_tensor(ref), stack=True)
        if 'refgt' in results:
            for i in range(len(results['refgt'])):
                ...
                if len(ref.shape) < 3:
                    ref = np.expand_dims(ref, -1)
                ref = np.ascontiguousarray(ref.transpose(2, 0, 1))
                results['refgt'][i] = DC(to_tensor(ref), stack=True)
        return results
